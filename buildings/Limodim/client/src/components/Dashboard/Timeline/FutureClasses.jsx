...
const FutureClasses = () => {
  ...
  useEffect(() => {
    api.getTimelineFutureClasses().then(res => setFuture(res.data));
  }, []);
  ...
  return (
    <section className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-full relative overflow-hidden">
      ...
      <div className="space-y-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-bold uppercase ${isStartingSoon ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-700'}`}>
                    {item.class_type === 'Tutorial' ? 'תרגול' : 'הרצאה'}
                  </span>
                </div>
        ...
        {future.length === 0 && (
          <div className="text-center py-6 text-slate-400 italic text-sm">אין שיעורים קרובים</div>
        )}
      </div>
    </section>
  );
