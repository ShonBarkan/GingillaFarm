...
const FutureClasses = () => {
  ...
  const isClassNow = (startTimeStr, endTimeStr, classDay) => {
    if (!startTimeStr || !endTimeStr || !isClassToday(classDay)) return false;
    ...
    <section className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-full relative overflow-hidden flex flex-col min-h-[250px]">
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-start pt-1">
        {future.length > 0 ? (
          <div className="space-y-3">
            {future.map((item, i) => {
              const isLive = isClassNow(item.time, item.end_time, item.day);
              const minsLeft = getMinutesUntil(item.time, item.day);
              const isStartingSoon = !isLive && minsLeft !== null && minsLeft > 0 && minsLeft <= 60;
              ...
              return (
                <div 
                  ...
                  className={`flex items-center justify-between p-3 rounded-xl border transition-all duration-300 ${
                    isLive 
                      ? 'border-emerald-500 shadow-lg ring-2 ring-emerald-500/10 bg-emerald-50 scale-[1.02] z-10'
                      : isStartingSoon 
                ...
                >
                  <div className="min-w-0 flex-1 text-right">
                    <div className="flex items-center gap-2 justify-start">
                      <Link 
                        ...
                        className={`font-black block hover:underline text-sm truncate ${
                            isLive ? 'text-emerald-700' : isStartingSoon ? 'text-blue-700' : 'text-blue-900'
                        }`}
                      >
                        {item.course_name}
                      </Link>
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-bold uppercase ${
                          isLive ? 'bg-emerald-600 text-white' : isStartingSoon ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-700'
                      }`}>
                        {item.class_type}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`text-[10px] font-bold ${
                          isLive ? 'text-emerald-600' : isStartingSoon ? 'text-blue-500' : 'text-blue-400'
                      }`}>
                        {isLive ? 'מתקיים כעת' : `יום ${item.day} ב-${item.time}`} | {item.location}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mr-4">
                    {isLive ? (
                      <div className="flex flex-col items-center justify-center bg-emerald-500 text-white px-3 py-1 rounded-lg shadow-md animate-pulse">
                        <span className="text-[10px] font-black leading-none">עכשיו</span>
                      </div>
                    ) : isStartingSoon ? (
                    ...
                    {item.zoom_link && (
                      <a href={item.zoom_link} target="_blank" rel="noopener noreferrer" 
                         className={`p-2 rounded-lg transition shadow-sm active:scale-90 ${
                             isLive ? 'bg-emerald-600 text-white' : isStartingSoon ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-600 hover:bg-blue-200'
                         }`}>
