...
const ExerciseWorkouts = ({ exerciseId }) => {
    ...
    useEffect(() => {
        const loadWorkouts = async () => {
            ...
            setIsLoading(true);
            try {
                // Get the current exercise and all its descendants IDs
                const descendantIds = mitamnimService.getRecursiveExerciseIds(exerciseId, allExercises);
                const relevantIds = [...new Set([...descendantIds, parseInt(exerciseId)])];

                // Fetch templates filtered by these IDs
                const data = await mitamnimService.getWorkoutTemplates({ 
                    parent_exercise_id: relevantIds 
                });

                // Client-side filtering to ensure strict matching
                const filtered = data.filter(t => 
                    t.parent_exercise_id === parseInt(exerciseId) || 
                    descendantIds.includes(t.parent_exercise_id)
                );
                ...
                {templates.map((template) => (
                    ...
                    >
                        <div className="flex flex-col min-w-0 flex-1 px-2">
                            ...
                            {template.parent_exercise_id !== parseInt(exerciseId) && (
