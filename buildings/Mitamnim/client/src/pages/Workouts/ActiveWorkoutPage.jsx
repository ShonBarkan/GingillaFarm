...
import { Save, XCircle, Plus, Search, X, Loader2 } from 'lucide-react';
...
const ActiveWorkoutPage = () => {
    ...
    useEffect(() => {
        const initWorkout = async () => {
            ...
            try {
                ...
                console.error("שגיאה באתחול האימון:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleAddManualExercise = async (exercise) => {
        try {
            ...
            console.error("שגיאה בהוספת תרגיל", e);
        }
    };
    ...
    const handleRemoveExercise = (index) => {
        if (window.confirm("האם להסיר את התרגיל מהאימון?")) {
            setWorkoutData(prev => ({
                ...prev,
    ...
    const handleCancel = () => {
        if (window.confirm("ביטול אימון? כל הנתונים שלא נשמרו יאבדו.")) {
            cancelWorkout();
    ...
    const onFinishConfirm = async (modalData) => {
        ...
        try {
            const finalTimestamp = modalData.retro_date;
            const allLogs = [];
                ...
                ex.sets.forEach(set => {
                    if (set.completed) {
                        allLogs.push({
                            ...
                            is_manual: false,
                            timestamp: finalTimestamp // שימוש בזמן שנבחר במודל
                        });
                    }
                });
            });
            ...
            await finishWorkout({
                ...modalData,
                duration: modalData.retro_duration_minutes
            }, allLogs);
            ...
            console.error("שגיאה בסיום האימון:", err);
    ...
    if (loading || !workoutData) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen gap-4">
                ...
                <p className="font-black text-gray-400 uppercase tracking-widest text-xs">טוען זירה...</p>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 md:p-8 pb-40" dir="rtl">
            <div className="sticky top-4 z-50 mb-10 px-2 md:px-4">
                <div className="bg-white/95 backdrop-blur-xl p-3 md:p-4 rounded-[3rem] shadow-2xl border border-gray-100 flex items-center justify-between gap-4 transition-all duration-300">
                    <div className="flex items-center gap-3 md:gap-5 border-l border-gray-100 pl-4 shrink-0">
                        <button onClick={handleCancel} className="p-2.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-all active:scale-90" title="בטל אימון">
                            <XCircle size={22} />
                        </button>
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => setIsAddExerciseOpen(true)} className="flex items-center gap-2 bg-gray-50 text-gray-700 px-3 md:px-4 py-2.5 rounded-2xl font-black border border-gray-100 hover:border-blue-200 hover:bg-white transition-all group active:scale-95 shadow-sm">
                        ...
                        <button onClick={() => setIsFinishModalOpen(true)} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2.5 md:px-6 md:py-3.5 rounded-2xl font-black shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95 shrink-0">
                            <Save size={16} />
                            <span className="text-[11px] md:text-sm">סיים אימון</span>
                        </button>
                    </div>
                </div>
            </div>

            <Reorder.Group axis="y" values={workoutData.exercises} onReorder={handleReorderExercises} className="space-y-6">
                <AnimatePresence initial={false} mode="popLayout">
                    {workoutData.exercises.map((exercise, idx) => (
                        <Reorder.Item key={exercise.instanceId} value={exercise} className="relative">
                            <ActiveExerciseCard 
                                exercise={exercise}
                                index={idx}
            ...
            <AnimatePresence>
                {isAddExerciseOpen && (
                    <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-0 z-[200] bg-white flex flex-col">
                        <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0">
                            <h3 className="text-2xl font-black text-gray-900 tracking-tighter">הוספת תרגיל</h3>
                            <button onClick={() => setIsAddExerciseOpen(false)} className="p-3 bg-gray-100 rounded-2xl text-gray-500 hover:bg-gray-200 transition-colors"><X size={20} /></button>
                        </div>
                        <div className="p-6 bg-gray-50/50">
                            <div className="relative group">
                                ...
                                <input type="text" placeholder="חיפוש תרגיל..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white border-2 border-transparent focus:border-blue-100 rounded-2xl py-4 pr-12 pl-4 font-bold text-gray-900 shadow-sm outline-none transition-all" />
                            </div>
                        </div>
                        <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
                            {filteredAvailable.map(ex => (
                                <button key={ex.id} onClick={() => handleAddManualExercise(ex)} className="w-full p-5 bg-white border border-gray-100 rounded-[1.5rem] flex items-center justify-between hover:border-blue-500 hover:bg-blue-50/30 transition-all text-right group shadow-sm active:scale-[0.98]">
                                    ...
                                    <div className="p-2 bg-gray-50 rounded-xl group-hover:bg-blue-600 group-hover:text-white transition-all"><Plus size={18} /></div>
                                </button>
                            ))}
                        </div>
                    </motion.div>
