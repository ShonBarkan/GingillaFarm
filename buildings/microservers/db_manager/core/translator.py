...
class SQLTranslator:
    ...
    def build_create_table(table: str, columns: Dict[str, str]):
        """
        Converts JSON schema to SQL.
        Example columns: {"name": "VARCHAR(255)", "age": "INTEGER"}
        """
        column_defs = [f"{name} {dtype}" for name, dtype in columns.items()]
        # Always add a default ID primary key for the Farm's records
    ...
    class SQLTranslator:
        # ... (Keep existing build_select, build_insert, build_create_table)
